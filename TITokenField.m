...
- (void)didEndEditing {
	...
	if (_removesTokensOnEndEditing){
		...
		if (_tokens.count){
			...
			CGSize untokSize = [untokenized sizeWithAttributes:@{@"font": [UIFont systemFontOfSize:14]}];
...
- (instancetype)initWithTitle:(NSString *)aTitle representedObject:(id)object font:(UIFont *)aFont {
	...
	if ((self = [super init])){
		...
		_font = aFont;
		_tintColor = [TIToken globalTintColor];
		_textColor = [UIColor whiteColor];
		_highlightedTextColor = [TIToken globalTintColor];
...
- (void)setTintColor:(UIColor *)newTintColor {
	...
	if (!newTintColor) newTintColor = [TIToken globalTintColor];
...
+ (UIColor *)globalTintColor {
    ...
    //rgb(44,156,198)
    return [UIColor colorWithRed:44.0f/255.0f green:156.0f/255.0f blue:198.0f/255.0f alpha:1.0f];
}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    CGSize titleSize = [_title boundingRectWithSize:CGSizeMake((_maxWidth - hTextPadding - accessoryWidth), CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: _font} context:nil].size;
...
- (void)drawRect:(CGRect)rect {
    ...
    [_title drawInRect:textBounds withAttributes:@{NSFontAttributeName: _font,NSForegroundColorAttributeName: drawHighlighted ? _highlightedTextColor : _textColor}];
}
